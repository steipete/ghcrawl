...
function seedRepoAndThreads(db: ReturnType<typeof openDb>): void {
  ...
  insertThread.run(
    ...
    10,
    "Fix cache collision",
    "https://github.com/openclaw/openclaw/pull/10",
    "h10",
  ...
  insertThread.run(
    ...
    11,
    "Fix cache collision",
    "https://github.com/openclaw/openclaw/pull/11",
    "h11",
    "2026-01-01T00:00:00Z",
    "2026-01-01T00:00:00Z",
    "2026-01-01T00:00:00Z",
...
test("persistent cluster store upserts edge evidence and governed memberships", () => {
  ...
  try {
    ...
    const membershipCount = db
      ...
      .get() as { count: number };
    const eventCount = db.prepare("select count(*) as count from cluster_events").get() as {
      count: number;
    };
    const run = db.prepare("select status from pipeline_runs where id = ?").get(runId) as {
      status: string;
    };
...
test("persistent cluster store records thread revisions and deterministic fingerprints", () => {
  ...
  try {
    ...
    const revisionCount = db.prepare("select count(*) as count from thread_revisions").get() as {
      count: number;
    };
...
test("persistent cluster store records code snapshots, changed files, and hunk signatures", () => {
  ...
  try {
    ...
    const snapshot = db
      .prepare(
        "select files_changed, additions, deletions, patch_digest, raw_diff_blob_id from thread_code_snapshots where id = ?",
      )
      .get(snapshotId) as {
    ...
    const file = db
      .prepare("select path, patch_blob_id from thread_changed_files where snapshot_id = ?")
      .get(snapshotId) as {
      path: string;
      patch_blob_id: number;
    };
    const hunkCount = db
      .prepare("select count(*) as count from thread_hunk_signatures where snapshot_id = ?")
...
test("persistent cluster store records structured key summaries", () => {
  ...
  try {
    ...
    const row = db
      .prepare("select input_hash, key_text from thread_key_summaries where thread_revision_id = ?")
      .get(revisionId) as {
      input_hash: string;
      key_text: string;
    };
