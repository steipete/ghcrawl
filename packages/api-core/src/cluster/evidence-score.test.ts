...
test("scoreSimilarityEvidence can improve confidence with optional enrichment", () => {
  const left = fp({
    id: 1,
    title: "Fix flaky download retry",
    body: "Retries forever after timeout.",
  });
  const right = fp({
    id: 2,
    title: "Handle stalled download timeout",
    body: "Retry loop never exits.",
  });

  const base = scoreSimilarityEvidence(left, right);
  const enriched = scoreSimilarityEvidence(left, right, {
    embeddingSimilarity: 0.95,
    llmKeySimilarity: 0.95,
  });
...
test("scoreSimilarityEvidence treats exact hunk overlap as strong evidence without prose similarity", () => {
  const left = fp({
    id: 1,
    title: "Replace queue scheduler",
    body: "Internal refactor.",
    hunks: ["same-hunk"],
  });
  const right = fp({
    id: 2,
    title: "Patch database migrator",
