...
test("minhash signatures are deterministic and comparable", () => {
  const first = minhashSignature(["cache", "key", "collision", "fix"], {
    permutations: 16,
    shingleSize: 2,
  });
  const second = minhashSignature(["cache", "key", "collision", "fix"], {
    permutations: 16,
    shingleSize: 2,
  });
  const different = minhashSignature(["ui", "button", "color", "fix"], {
    permutations: 16,
    shingleSize: 2,
  });
