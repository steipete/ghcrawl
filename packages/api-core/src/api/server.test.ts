...
test("runs endpoint returns recent pipeline history", async () => {
  ...
  try {
    ...
    const response = await fetch(
      `http://127.0.0.1:${address.port}/runs?owner=openclaw&repo=openclaw&kind=sync`,
    );
    assert.equal(response.status, 200);
...
test("threads endpoint can filter by a bulk number list", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      payload.threads.map((thread) => thread.number),
      [44, 42],
    );
  } finally {
    await new Promise<void>((resolve, reject) =>
...
test("close-thread and includeClosed thread routes expose locally closed items", async () => {
  ...
  try {
    ...
    const defaultResponse = await fetch(
      `http://127.0.0.1:${address.port}/threads?owner=openclaw&repo=openclaw`,
    );
    ...
    const includeClosedPayload = threadsResponseSchema.parse(
      (await includeClosedResponse.json()) as unknown,
    );
...
test("exclude cluster member action records a durable override", async () => {
  ...
  try {
    ...
    const response = await fetch(
      `http://127.0.0.1:${address.port}/actions/exclude-cluster-member`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
...
test("durable clusters endpoint returns stable cluster state", async () => {
  ...
  try {
    ...
    const response = await fetch(
      `http://127.0.0.1:${address.port}/durable-clusters?owner=openclaw&repo=openclaw`,
    );
    assert.equal(response.status, 200);
...
test("cluster summary and detail endpoints return contract payloads", async () => {
  ...
  try {
    ...
    const summaries = clusterSummariesResponseSchema.parse(
      (await summariesResponse.json()) as unknown,
    );
