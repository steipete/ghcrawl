...
import {
  explainStoredDurableCluster,
  listStoredDurableClusters,
} from "./cluster/durable-queries.js";
import {
...
import {
  loadClusterableActiveVectorMeta,
  loadClusterableThreadMeta,
  loadNormalizedActiveVectors,
} from "./embedding/clusterable.js";
import {
...
import { cosineSimilarity, rankNearestNeighborsByScore } from "./search/exact.js";
...
import {
  ...
  countStaleOpenThreads,
  reconcileMissingOpenThreads,
...
import {
  getTuiRepoStats,
  getTuiRepositoryRefreshState,
  type TuiEmbeddingStatsMode,
} from "./tui/repo-stats.js";
import { buildTuiThreadDetail } from "./tui/thread-detail.js";
import {
...
import {
  getVectorliteClusterQuery,
  normalizedDistanceToScore,
  normalizedEmbeddingBuffer,
  parseStoredVector,
  vectorBlob,
} from "./vector/encoding.js";
import {
...
import {
  isCorruptedVectorIndexError,
  repositoryVectorStorePath,
} from "./vector/repository-store.js";
...
export class GHCrawlService {
  ...
  constructor(
    ...
    this.github =
      options.github ??
      (this.config.githubToken ? makeGitHubClient({ token: this.config.githubToken }) : undefined);
    this.ai =
      options.ai ??
      (this.config.openaiApiKey ? new OpenAiProvider(this.config.openaiApiKey) : undefined);
  ...
  closeClusterLocally(params: { owner: string; repo: string; clusterId: number }): CloseResponse {
    ...
    const row = this.db
      .prepare(
        "select id, representative_thread_id from clusters where repo_id = ? and cluster_run_id = ? and id = ? limit 1",
      )
      .get(repository.id, latestRun.id, params.clusterId) as
      | { id: number; representative_thread_id: number | null }
      | undefined;
    if (!row) {
    ...
    this.db.transaction(() => {
      durableClusterId = this.ensureDurableClusterForRunCluster(
        repository.id,
        row.id,
  ...
  excludeThreadFromCluster(params: ExcludeClusterMemberRequest): ClusterOverrideResponse {
    ...
    const existingMembership = this.db
      .prepare(
        "select role, score_to_representative from cluster_memberships where cluster_id = ? and thread_id = ? limit 1",
      )
      .get(cluster.id, thread.id) as
      | { role: "canonical" | "duplicate" | "related"; score_to_representative: number | null }
      | undefined;
  ...
  includeThreadInCluster(params: IncludeClusterMemberRequest): ClusterOverrideResponse {
    ...
    this.db.transaction(() => {
      this.db
        .prepare(
          "delete from cluster_overrides where cluster_id = ? and thread_id = ? and action = 'exclude'",
        )
      ...
      this.db
        .prepare(
          "update cluster_memberships set added_by = 'user', updated_at = ? where cluster_id = ? and thread_id = ?",
        )
  ...
  setClusterCanonicalThread(params: SetClusterCanonicalRequest): ClusterOverrideResponse {
    ...
    const membership = this.db
      .prepare(
        "select score_to_representative from cluster_memberships where cluster_id = ? and thread_id = ? limit 1",
      )
      .get(cluster.id, thread.id) as { score_to_representative: number | null } | undefined;
    if (!membership) {
      throw new Error(
        `Thread #${params.threadNumber} is not a member of durable cluster ${cluster.id}.`,
      );
    }
    ...
    this.db.transaction(() => {
      ...
      this.db
        .prepare(
          "update cluster_groups set representative_thread_id = ?, updated_at = ? where id = ?",
        )
        .run(thread.id, timestamp, cluster.id);
      this.db
        .prepare(
          "update cluster_memberships set role = 'related', updated_at = ? where cluster_id = ? and role = 'canonical'",
        )
  ...
  mergeDurableClusters(params: MergeClustersRequest): ClusterMergeResponse {
    ...
    const clusters = this.db
      ...
      .all(repository.id, params.sourceClusterId, params.targetClusterId) as Array<{
      id: number;
      stable_slug: string;
    }>;
    ...
    if (!source) {
      throw new Error(
        `Durable source cluster ${params.sourceClusterId} was not found for ${repository.fullName}.`,
      );
    }
    if (!target) {
      throw new Error(
        `Durable target cluster ${params.targetClusterId} was not found for ${repository.fullName}.`,
      );
    }
    ...
    this.db.transaction(() => {
      ...
      for (const member of members) {
        ...
        this.db
          ...
          .run(
            ...
            target.id,
            member.thread_id,
            params.reason ?? `merged from cluster ${source.id}`,
            timestamp,
          );
        upsertClusterMembership(this.db, {
          clusterId: target.id,
      ...
      this.db
        .prepare(
          "update cluster_groups set status = 'merged', closed_at = ?, updated_at = ? where id = ?",
        )
  ...
  splitDurableCluster(params: SplitClusterRequest): ClusterSplitResponse {
    const threadNumbers = Array.from(new Set(params.threadNumbers)).sort(
    ...
    const source = this.db
      ...
      .get(repository.id, params.sourceClusterId) as
      | { id: number; stable_slug: string }
      | undefined;
    if (!source) {
      throw new Error(
        `Durable source cluster ${params.sourceClusterId} was not found for ${repository.fullName}.`,
    ...
    const requestedThreads = this.db
      .prepare(
        `select id, number, title from threads where repo_id = ? and number in (${placeholders})`,
      )
    ...
    if (missingNumbers.length > 0) {
      throw new Error(
        `Thread(s) ${missingNumbers.map((number) => `#${number}`).join(", ")} were not found for ${repository.fullName}.`,
      );
    }
    ...
    const selectedMembers = activeMembers.filter((member) =>
      selectedThreadIds.has(member.thread_id),
    );
    const missingActiveNumbers = threadNumbers.filter(
      (number) => !selectedMembers.some((member) => member.number === number),
    );
    if (missingActiveNumbers.length > 0) {
      throw new Error(
        `Thread(s) ${missingActiveNumbers.map((number) => `#${number}`).join(", ")} are not active members of durable cluster ${source.id}.`,
      );
    }

    const remainingMembers = activeMembers.filter(
      (member) => !selectedThreadIds.has(member.thread_id),
    );
    ...
    const selectedCanonical =
      selectedMembers.find((member) => member.role === "canonical") ?? selectedMembers[0];
    const remainingCanonical =
      remainingMembers.find((member) => member.role === "canonical") ?? remainingMembers[0];
    if (!selectedCanonical || !remainingCanonical) {
      throw new Error("Split requires selected and remaining active members.");
    }

    const identity = humanKeyForValue(
      `cluster-split:${repository.id}:${source.id}:${selectedMembers.map((member) => member.thread_id).join(",")}`,
    );
    const timestamp = nowIso();
    let newClusterId = 0;
    this.db.transaction(() => {
      ...
      this.db
        .prepare(
          "update cluster_memberships set role = 'canonical', updated_at = ? where cluster_id = ? and thread_id = ?",
        )
      ...
      for (const member of selectedMembers) {
        ...
        upsertClusterMembership(this.db, {
          ...
          scoreToRepresentative:
            member.thread_id === selectedCanonical.thread_id ? 1 : member.score_to_representative,
          addedBy: "user",
          addedReason: {
            source: "splitDurableCluster",
            sourceClusterId: source.id,
  ...
  async syncRepository(params: SyncOptions): Promise<SyncResultDto> {
    ...
    const reporter = params.onProgress
      ? (message: string) => params.onProgress?.(message.replace(/^\[github\]/, "[sync/github]"))
      : undefined;
    ...
    const overlapReferenceAt =
      syncCursor.lastOverlappingOpenScanCompletedAt ?? syncCursor.lastFullOpenScanStartedAt;
    const effectiveSince =
      params.since ??
      (params.limit === undefined && overlapReferenceAt
        ? deriveIncrementalSince(overlapReferenceAt, crawlStartedAt)
        : undefined);
    const isFullOpenScan =
      params.limit === undefined && params.since === undefined && overlapReferenceAt === null;
    const isOverlappingOpenScan =
      params.limit === undefined &&
      overlapReferenceAt !== null &&
      effectiveSince !== undefined &&
      (parseIso(effectiveSince) ?? Number.POSITIVE_INFINITY) <=
        (parseIso(overlapReferenceAt) ?? Number.NEGATIVE_INFINITY);

    try {
      params.onProgress?.(
        `[sync] listing issues and pull requests for ${params.owner}/${params.repo}`,
      );
      params.onProgress?.(
        includeComments
      ...
      if (isFullOpenScan) {
        params.onProgress?.(
          "[sync] full open scan; no prior completed overlap/full cursor was found for this repository",
        );
      ...
      const items = await github.listRepositoryIssues(
        params.owner,
        params.repo,
        effectiveSince,
        params.limit,
      ...
      for (const [index, item] of items.entries()) {
        if ((includeComments || includeCode) && index > 0 && index % SYNC_BATCH_SIZE === 0) {
          params.onProgress?.(
            `[sync] batch boundary reached at ${index} threads; sleeping 5s before continuing`,
          );
        ...
        try {
          ...
          const threadPayload = shouldFetchPullPayload
            ? await github.getPull(params.owner, params.repo, number, reporter)
            : item;
      ...
      if (!shouldSweepClosedOverlap) {
        params.onProgress?.(
          "[sync] skipping closed overlap sweep because this scan has no overlap window",
        );
      }
      const threadsClosedFromClosedSweep = shouldSweepClosedOverlap
        ...
        : 0;
      const canFullReconcile =
        params.fullReconcile === true &&
        params.limit === undefined &&
        (isFullOpenScan || isOverlappingOpenScan);
      ...
      if (!canFullReconcile && params.fullReconcile !== true) {
        params.onProgress?.(
          "[sync] skipping full stale-open reconciliation by default; use --full-reconcile to force direct checks of all unseen open items",
        );
      } else if (!canFullReconcile) {
        params.onProgress?.(
          "[sync] skipping full stale-open reconciliation because this scan did not overlap a confirmed full/overlap cursor",
        );
      ...
      const reconciledOpenCloseAt =
        shouldSweepClosedOverlap || shouldReconcileMissingOpenThreads ? finishedAt : null;
      const nextSyncCursor: SyncCursorState = {
        lastFullOpenScanStartedAt: isFullOpenScan
          ? crawlStartedAt
          : syncCursor.lastFullOpenScanStartedAt,
        lastOverlappingOpenScanCompletedAt: isOverlappingOpenScan
          ? finishedAt
          : syncCursor.lastOverlappingOpenScanCompletedAt,
        lastNonOverlappingScanCompletedAt:
          !isFullOpenScan && !isOverlappingOpenScan
            ? finishedAt
            : syncCursor.lastNonOverlappingScanCompletedAt,
      ...
      finishServiceRun(
        ...
        "completed",
        {
          ...
          threadsClosedFromClosedBackfill,
          threadsClosedFromDirectReconcile,
          directReconcileSkippedStaleThreadCount:
            canFullReconcile && !shouldReconcileMissingOpenThreads
  ...
  async summarizeRepository(params: {
    ...
    const repository = this.requireRepository(params.owner, params.repo);
    const runId = startServiceRun(
      this.db,
      "summary_runs",
      repository.id,
      params.threadNumber ? `thread:${params.threadNumber}` : repository.fullName,
    );
    const includeComments = params.includeComments ?? false;
    ...
    try {
      ...
      params.onProgress?.(
        `[summarize] loaded ${rows.length} candidate thread(s) for ${repository.fullName}`,
      );
      params.onProgress?.(
        includeComments
      ...
      const pending = sources.filter((row) => {
        const latest = this.db
          ...
          | undefined;
        return (
          latest?.content_hash !== row.summaryContentHash ||
          latest?.prompt_version !== SUMMARY_PROMPT_VERSION
        );
      });
      ...
      const writer = new IterableMapper(
        ...
        async ({ row, result }) => {
          ...
          this.upsertSummary(
            ...
            summary.maintainerSignalSummary,
          );
          this.upsertSummary(
            row.id,
            row.summaryContentHash,
      ...
      finishServiceRun(this.db, "summary_runs", runId, "completed", {
        summarized,
        inputTokens,
        outputTokens,
        totalTokens,
  ...
  async generateKeySummaries(params: {
    ...
    const runId = startServiceRun(
      ...
      repository.id,
      params.threadNumber
        ? `key-summary:${params.threadNumber}`
        : `key-summary:${repository.fullName}`,
    );

    try {
      let sql = `select id, number, title, body, labels_json, raw_json, updated_at_gh
         from threads
      ...
      params.onProgress?.(
        `[key-summary] loaded ${rows.length} candidate thread(s) for ${repository.fullName}`,
      );
      ...
      for (const row of rows) {
        ...
        const existing = this.db
          ...
          ) as { input_hash: string } | undefined;
  ...
  purgeComments(params: {
    ...
    params.onProgress?.(
      `[purge-comments] removing hydrated comments from ${threads.length} thread(s) in ${repository.fullName}`,
    );
    ...
    try {
      if (params.threadNumber === undefined) {
        if (!isRepoVectorStateCurrent(this.db, this.config, repository.id)) {
          ...
          if (pruned > 0) {
            params.onProgress?.(
              `[embed] pruned ${pruned} closed or inactive vector(s) before refresh`,
            );
          }
        }
      }
      ...
      let embedded = 0;
      const batches = chunkEmbeddingTasks(
        pending,
        this.config.embedBatchSize,
        EMBED_MAX_BATCH_TOKENS,
      );
      const mapper = new IterableMapper(
        batches,
      ...
      for await (const batchResult of mapper) {
        ...
        const estimatedTokens = batchResult.reduce(
  ...
  async clusterRepository(params: {
    ...
    const repository = this.requireRepository(params.owner, params.repo);
    const runSubject = params.threadNumber
      ? `${repository.fullName}#${params.threadNumber}`
      : repository.fullName;
    ...
    const pipelineRunId = createPipelineRun(this.db, {
      ...
      configHash: stableContentHash(
        JSON.stringify({
          ...
          crossKindMinScore: Math.max(
            params.minScore ?? DEFAULT_CLUSTER_MIN_SCORE,
            DEFAULT_CROSS_KIND_CLUSTER_MIN_SCORE,
          ),
          k: params.k ?? 16,
          embedModel: this.config.embedModel,
    ...
    try {
      ...
      if (params.threadNumber && !seedThread) {
        throw new Error(
          `Open thread #${params.threadNumber} was not found for ${repository.fullName}.`,
        );
      }
      ...
      const fingerprintItems = seedThreadIds
        ? deterministicItems.filter((item) => seedThreadIds.includes(item.id))
        : deterministicItems;
      materializeLatestDeterministicFingerprints(this.db, fingerprintItems, params.onProgress);
      ...
      mergeSourceKindEdges(
        ...
        deterministic.edges
          ...
          .map((edge) => ({
            ...
            score: Math.max(
              edge.score,
              edge.tier === "strong" ? 0.94 : Math.min(0.86, minScore + 0.04),
            ),
          })),
      ...
      const vectorItems = loadClusterableActiveVectorMeta({
        db: this.db,
        config: this.config,
        repoId: repository.id,
      });
      if (vectorItems.length > 0) {
        const queryVectorItems = seedThreadIds
          ? vectorItems.filter((item) => seedThreadIds.includes(item.id))
          : vectorItems;
        ...
        for (const item of queryVectorItems) {
          ...
          if (params.onProgress && now - lastProgressAt >= CLUSTER_PROGRESS_INTERVAL_MS) {
            params.onProgress(
              `[cluster] queried ${processed}/${queryVectorItems.length} vectors current_edges=${aggregatedEdges.size}`,
            );
            lastProgressAt = now;
          }
        }
        ...
        !seedThreadIds &&
        hasLegacyEmbeddings(this.db, this.config.embedModel, repository.id)
      ) {
        ...
        for (const legacyEdge of legacyEdges.values()) {
          for (const sourceKind of legacyEdge.sourceKinds) {
            mergeSourceKindEdges(
              ...
              [
                {
                  leftThreadId: legacyEdge.leftThreadId,
                  rightThreadId: legacyEdge.rightThreadId,
                  score: legacyEdge.score,
                },
              ],
              sourceKind,
            );
      ...
      const droppedCrossKindEdges = pruneWeakCrossKindEdges(
        aggregatedEdges,
        threadKinds,
        crossKindMinScore,
      );
      ...
      const clusterItems = seedThreadIds
        ? deterministicItems.filter((item) => involvedIds.has(item.id))
        : deterministicItems;
      ...
      if (!seedThreadIds && vectorStateCurrent) {
        ...
        this.cleanupMigratedRepositoryArtifacts(
          repository.id,
          repository.fullName,
          params.onProgress,
        );
      ...
      finishPipelineRun(this.db, pipelineRunId, {
        status: "failed",
  ...
  clusterExperiment(params: {
    ...
    const activeVectors = isRepoVectorStateCurrent(this.db, this.config, repository.id)
      ...
      : [];
    const activeSourceKind = activeVectorSourceKind(this.config.embeddingBasis);
    const useActiveVectors =
      activeVectors.length > 0 && (params.sourceKinds === undefined || loaded.items.length === 0);
    const sourceKinds = useActiveVectors
      ? [activeSourceKind]
      : (params.sourceKinds ?? loaded.sourceKinds);
    ...
    try {
      if (backend === "exact") {
        if (useActiveVectors) {
          ...
          const normalizedRows = activeVectors.map(({ id, embedding }) => ({
            id,
          ...
          const totalItems = sourceKinds.reduce(
            (sum, sourceKind) =>
              sum +
              countEmbeddingsForSourceKind({ db: this.db, repoId: repository.id, sourceKind }),
            0,
          );
        ...
        (
          tempDb as SqliteDatabase & { loadExtension: (extensionPath: string) => void }
        ).loadExtension(vectorlite.vectorlitePath());
          ...
          ? [
              {
                ...
                rows: activeVectors.map(({ id, embedding }) => ({
                  id,
                  normalizedEmbedding: embedding,
                })),
              },
        ...
        for (const source of vectorSources) {
          ...
          for (const row of source.rows) {
            const candidates = query.all(
              normalizedEmbeddingBuffer(row.normalizedEmbedding),
            ) as Array<{
              rowid: number;
              distance: number;
            }>;
            ...
            for (const candidate of ranked) {
              ...
              if (existing) {
                existing.scores.set(
                  source.sourceKind,
                  Math.max(existing.scores.get(source.sourceKind) ?? -1, score),
                );
                continue;
              }
      ...
      const defaultWeights: Record<EmbeddingSourceKind, number> = {
        dedupe_summary: 0.5,
        llm_key_summary: 0.5,
        title: 0.3,
        body: 0.2,
      ...
      const clusterNodes = items.map((item) => ({
        threadId: item.id,
        number: item.number,
        title: item.title,
      }));
      ...
      const clusterMode =
        params.clusterMode ?? (params.maxClusterSize !== undefined ? "refine" : "basic");
      const clusters =
        clusterMode === "bounded"
  ...
  async searchRepository(params: {
    ...
    if (mode !== "semantic") {
      ...
      const rows = this.db
        ...
        .all(repository.id, keywordQuery, keywordQuery, limit * 2) as Array<{
        thread_id: number;
        rank: number;
      }>;
      for (const row of rows) {
    ...
    if (mode !== "keyword" && this.ai) {
      if (isRepoVectorStateCurrent(this.db, this.config, repository.id)) {
        ...
        for (const neighbor of neighbors) {
          ...
          semanticScores.set(
            neighbor.threadId,
            Math.max(semanticScores.get(neighbor.threadId) ?? -1, neighbor.score),
          );
        }
        ...
        for (const row of iterateStoredEmbeddings({
          db: this.db,
          repoId: repository.id,
          embedModel: this.config.embedModel,
        })) {
    ...
    const hits = threadRows
      .map((row) => {
        ...
        return {
          ...
          semanticScore,
          hybridScore,
          neighbors: (neighborsByThread.get(row.id) ?? [])
  ...
  listNeighbors(params: {
    ...
    if (targetRow) {
      ...
      const aggregated = new Map<
        number,
        { number: number; kind: "issue" | "pull_request"; title: string; score: number }
      >();
      for (const row of iterateStoredEmbeddings({
        db: this.db,
        repoId: repository.id,
  ...
  optimizeStorage(params: { owner?: string; repo?: string } = {}): OptimizeResponse {
    const repository =
      params.owner && params.repo ? this.requireRepository(params.owner, params.repo) : null;
    ...
    const outputPath = path.resolve(
      params.outputPath ??
        path.join(
          ...
          "exports",
          `${repository.owner}__${repository.name}.sync.db`,
        ),
    );
  ...
  getClusterDetailDump(params: {
    ...
    if (!cluster) {
      throw new Error(
        `Cluster ${params.clusterId} was not found for ${snapshot.repository.fullName}.`,
      );
    }
    ...
    const members = detail.members
      .slice(0, params.memberLimit ?? detail.members.length)
      .map((member) => {
  ...
  getTuiSnapshot(params: {
    ...
    const rawClusters = latestRun
      ? listRawTuiClusters(this.db, repository.id, latestRun.id, minSize)
      : [];
  ...
  getTuiClusterDetail(params: {
    ...
    const repository = this.requireRepository(params.owner, params.repo);
    const clusterRunId =
      params.clusterRunId ?? getLatestClusterRun(this.db, repository.id)?.id ?? null;

    const summary = clusterRunId
      ? getRawTuiClusterSummary(this.db, repository.id, clusterRunId, params.clusterId)
      : null;
    const durableSummary = summary
      ? null
      : getDurableTuiClusterSummary(this.db, repository.id, params.clusterId);
    ...
    return {
      ...
      members: listTuiClusterMembers(
        this.db,
        params.clusterId,
        summary ? "run_cluster" : "durable_cluster",
      ),
    };
  }
  ...
  private cleanupMigratedRepositoryArtifacts(
    repoId: number,
    repoFullName: string,
    onProgress?: (message: string) => void,
  ): void {
  ...
  private ensureDurableClusterForRunCluster(
    repoId: number,
    runClusterId: number,
    ...
    const identity = humanKeyForValue(
      `repo:${repoId}:cluster-representative:${resolvedRepresentativeThreadId}`,
    );
    const memberIds = members.map((member) => member.thread_id);
    const durableIdentity = this.resolveDurableClusterIdentity(
      repoId,
      identity.hash,
      memberIds,
      new Set(),
    ...
    for (const member of members) {
      upsertClusterMembership(this.db, {
        ...
        state: "active",
        scoreToRepresentative:
          member.thread_id === resolvedRepresentativeThreadId ? 1 : member.score_to_representative,
        addedBy: "algo",
        addedReason: {
  ...
  private requireRepository(owner: string, repo: string): RepositoryDto {
    ...
    const row = this.db
      .prepare("select * from repositories where full_name = ? limit 1")
  ...
  private async aggregateRepositoryEdges(
    ...
    const totalItems = sourceKinds.reduce(
      (sum, sourceKind) => sum + countEmbeddingsForSourceKind({ db: this.db, repoId, sourceKind }),
      0,
    );
    ...
    const shouldParallelize =
      ...
      totalItems >= CLUSTER_PARALLEL_MIN_EMBEDDINGS &&
    ...
    const progressBySource = new Map<
      EmbeddingSourceKind,
      { processedItems: number; totalItems: number; currentEdgeEstimate: number }
    >();

    const edgeSets = await Promise.all(
      sourceKinds.map(
        (sourceKind) =>
          new Promise<Array<{ leftThreadId: number; rightThreadId: number; score: number }>>(
            (resolve, reject) => {
              ...
              worker.on("message", (message: unknown) => {
                ...
                if (typed.type === "progress") {
                  ...
                  if (params.onProgress) {
                    const processedItems = Array.from(progressBySource.values()).reduce(
                      (sum, value) => sum + value.processedItems,
                      0,
                    );
                    const currentEdgeEstimate = Array.from(progressBySource.values()).reduce(
                      (sum, value) => sum + value.currentEdgeEstimate,
                      0,
                    );
                    params.onProgress(
  ...
  private persistClusterRun(
    ...
    this.db.transaction(() => {
      this.db
        .prepare(
          "delete from cluster_members where cluster_id in (select id from clusters where cluster_run_id = ?)",
        )
        .run(runId);
      ...
      for (const cluster of clusters) {
        ...
        for (const memberId of cluster.members) {
          ...
          const score =
            memberId === cluster.representativeThreadId
              ? null
              : (aggregatedEdges.get(key)?.score ?? null);
  ...
  private persistDurableClusterState(
    ...
    this.db.transaction(() => {
      ...
      for (const cluster of clusters) {
        const identity = humanKeyForValue(
          `repo:${repoId}:cluster-representative:${cluster.representativeThreadId}`,
        );
        const durableIdentity = this.resolveDurableClusterIdentity(
          repoId,
          identity.hash,
          cluster.members,
          claimedDurableClusterIds,
        );
        ...
        for (const memberId of cluster.members) {
          ...
          const score =
            memberId === representativeThreadId
              ? 1
              : (aggregatedEdges.get(scoreKey)?.score ?? null);
          ...
          upsertClusterMembership(this.db, {
            ...
            runId: pipelineRunId,
            addedBy:
              memberId === representativeThreadId && forcedCanonical?.thread_id === memberId
                ? "user"
                : "algo",
            addedReason: {
              source: "clusterRepository",
        ...
        for (const forced of forcedIncludes) {
          ...
          const score =
            forced.thread_id === representativeThreadId
              ? 1
              : (aggregatedEdges.get(scoreKey)?.score ?? null);
          upsertClusterMembership(this.db, {
            clusterId,
  ...
  private resolveDurableClusterIdentity(
    ...
    const exact = this.db
      ...
      .get(repoId, representativeStableKey) as
      | { id: number; stable_key: string; stable_slug: string }
      | undefined;
      ...
      rows
        ...
        .map((row) => {
          ...
          return {
            ...
            latestMembershipTime: row.latest_membership_updated_at
              ? Date.parse(row.latest_membership_updated_at)
              : 0,
          };
        })
  ...
  private pruneOldClusterRuns(repoId: number, keepRunId: number): void {
    this.db
      .prepare("delete from cluster_runs where repo_id = ? and id <> ?")
