...
export class GHCrawlService {
  ...
  exportPortableSync(params: PortableSyncExportOptions): PortableSyncExportResponse {
    ...
    try {
      ...
      this.createPortableSyncSchema(out);
      out.exec(`attach database ${sqlStringLiteral(sourcePath)} as source`);
      this.populatePortableSyncDb(out, { repoId: repository.id, sourcePath, bodyChars });
    ...
    const sourceBytes = fs.statSync(sourcePath).size + this.fileSize(`${sourcePath}-wal`) + this.fileSize(`${sourcePath}-shm`);
    const verify = openDb(outputPath);
    try {
      verify.pragma('journal_mode = DELETE');
      const tables = portableTables.map((name) => ({ name, rows: this.countPortableRows(verify, name) }));
      return {
        ok: true,
        repository: {
          id: repository.id,
  ...
  private createPortableSyncSchema(db: SqliteDatabase): void {
    db.exec(`
  ...
  private populatePortableSyncDb(db: SqliteDatabase, params: { repoId: number; sourcePath: string; bodyChars: number }): void {
    const exportedAt = nowIso();
    const insertMetadata = db.prepare('insert into portable_metadata (key, value) values (?, ?)');
    insertMetadata.run('schema', 'ghcrawl-portable-sync-v1');
