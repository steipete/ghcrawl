...
test("migrate creates core tables", () => {
  ...
  try {
    ...
    const summaryColumns = db.prepare("pragma table_info(document_summaries)").all() as Array<{
      name: string;
    }>;
    assert.ok(summaryColumns.map((column) => column.name).includes("prompt_version"));

    const clusterMembershipColumns = db
      .prepare("pragma table_info(cluster_memberships)")
      .all() as Array<{ name: string }>;
