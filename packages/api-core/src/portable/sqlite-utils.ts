...
export function readPortableMetadata(db: SqliteDatabase): Record<string, string> {
  const rows = db.prepare("select key, value from portable_metadata order by key").all() as Array<{
    key: string;
    value: string;
  }>;
...
export function readDbstatSizes(
  db: SqliteDatabase,
): Array<{ name: string; bytes: number | null; rows: number | null }> {
  try {
    const rows = db
      .prepare(
        `select
    ...
    return rows.map((row) => ({
      name: row.name,
      bytes: row.bytes,
      rows: safeCountRows(db, row.name),
    }));
  } catch {
...
export function attachedTableHasColumn(
  db: SqliteDatabase,
  schemaName: string,
  tableName: string,
  columnName: string,
): boolean {
  const rows = db.prepare(`pragma ${schemaName}.table_info("${tableName}")`).all() as Array<{
    name: string;
  }>;
