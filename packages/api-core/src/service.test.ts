...
function makeTestConfig(
  overrides: Partial<GHCrawlService["config"]> = {},
): GHCrawlService["config"] {
  ...
  try {
    ...
    service.db
      ...
      .run(
        ...
        "input-hash",
        "output-hash",
        "intent: fix gateway crash\nsurface: startup\nmechanism: guard config",
        now,
      );
    service.db
      .prepare(
    ...
    service.db
      .prepare(
        "insert into pipeline_runs (id, repo_id, run_kind, status, started_at) values (?, ?, 'cluster', 'completed', ?)",
      )
      .run(60, 1, now);
    service.db
      .prepare(
    ...
    try {
      driftDb
        .prepare(
          "update threads set title = 'Gateway crash from stale portable copy' where number = 42",
        )
        .run();
    } finally {
      driftDb.close();
    ...
    try {
      const thread = portable
        .prepare("select body_excerpt, body_length from threads where number = 42")
        .get() as {
        body_excerpt: string;
        body_length: number;
      };
      ...
      const bulkyTables = portable
        .prepare(
          "select name from sqlite_master where type = 'table' and name in ('documents', 'comments', 'blobs', 'thread_vectors', 'cluster_events')",
        )
        .all() as Array<{ name: string }>;
      assert.deepEqual(bulkyTables, []);
      const summaryCount = portable
        .prepare("select count(*) as count from thread_key_summaries")
        .get() as { count: number };
      const membershipCount = portable
        .prepare("select count(*) as count from cluster_memberships")
        .get() as { count: number };
    ...
    const sourceThread = service.db
      .prepare("select raw_json, body from threads where id = 10")
      .get() as {
      raw_json: string;
      body: string;
    };
    ...
    try {
      ...
      const importedThread = importService.db
        .prepare("select body, raw_json from threads where number = 42")
        .get() as {
        body: string;
        raw_json: string;
      };
      ...
      const importedPortableOnlyThread = importService.db
        .prepare("select state, body, raw_json from threads where number = 44")
        .get() as {
        state: string;
        body: string;
        raw_json: string;
      ...
      const importedMembership = importService.db
        .prepare(
          "select first_seen_run_id, last_seen_run_id from cluster_memberships where cluster_id = 1 and thread_id = 1",
        )
    ...
    try {
      ...
      assert.equal(importResult.ok, true);
      const importedThread = richImportService.db
        .prepare("select body, raw_json from threads where number = 42")
        .get() as {
        body: string;
        raw_json: string;
...
test("listRunHistory returns recent runs across pipeline tables", () => {
  ...
  try {
    ...
    service.db
      .prepare(
        `insert into sync_runs (id, repo_id, scope, status, started_at, finished_at, stats_json) values (?, ?, ?, ?, ?, ?, ?)`,
      )
      .run(
        1,
        1,
    ...
    service.db
      .prepare(
        `insert into cluster_runs (id, repo_id, scope, status, started_at, finished_at, error_text) values (?, ?, ?, ?, ?, ?, ?)`,
      )
      .run(
        2,
        1,
    ...
    assert.deepEqual(
      syncRuns.runs.map((run) => run.runKind),
      ["sync"],
    );
  } finally {
    service.close();
...
test("syncRepository defaults to metadata-only mode, preserves thread kind, and tracks first/last pull timestamps", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      service
        .listThreads({ owner: "openclaw", repo: "openclaw", numbers: [43, 42, 999] })
        .threads.map((thread) => thread.number),
      [43, 42],
    );
    ...
    const fingerprintCount = service.db
      .prepare("select count(*) as count from thread_fingerprints")
      .get() as { count: number };
    assert.equal(fingerprintCount.count, 2);

    const rows = service.db
      .prepare(
        "select number, kind, first_pulled_at, last_pulled_at from threads order by number asc",
      )
      .all() as Array<{
      number: number;
...
test("syncRepository fetches comments, reviews, and review comments when includeComments is enabled", async () => {
  ...
  try {
    ...
    const commentCount = service.db.prepare("select count(*) as count from comments").get() as {
      count: number;
    };
    assert.equal(commentCount.count, 3);
    const largeComment = service.db
      .prepare(
        "select raw_json, raw_json_blob_id from comments where comment_type = 'issue_comment' limit 1",
      )
    ...
    const blob = service.db
      .prepare("select storage_kind from blobs where id = ?")
      .get(largeComment.raw_json_blob_id) as { storage_kind: string };
...
test("syncRepository hydrates pull request code snapshots when includeCode is enabled", async () => {
  ...
  try {
    ...
    const snapshot = service.db
      .prepare("select base_sha, head_sha, files_changed from thread_code_snapshots")
      .get() as {
      base_sha: string;
      head_sha: string;
      files_changed: number;
    };
    const file = service.db.prepare("select path from thread_changed_files").get() as {
      path: string;
    };
    const hunkCount = service.db
      .prepare("select count(*) as count from thread_hunk_signatures")
      .get() as { count: number };
    const fingerprint = service.db
      .prepare("select feature_json from thread_fingerprints")
      .get() as { feature_json: string };
...
test("syncRepository skips comment/code hydration and fingerprint refresh when a PR closes during sync", async () => {
  ...
  try {
    ...
    const thread = service.db
      .prepare(
        "select state, closed_at_gh, merged_at_gh from threads where number = 43 and kind = 'pull_request'",
      )
    ...
    const snapshotCount = service.db
      .prepare("select count(*) as count from thread_code_snapshots")
      .get() as { count: number };
    const fingerprintCount = service.db
      .prepare("select count(*) as count from thread_fingerprints")
      .get() as { count: number };
  ...
  try {
    ...
    service.db
      ...
      .run(
        ...
        "human",
        "User",
        "This extra comment should stay out.",
        0,
        "{}",
        now,
        now,
...
test("summarizeRepository prices progress output using the configured summary model", async () => {
  ...
  try {
    ...
    assert.ok(
      progress.some(
        (message) => message.includes("cost=$0.25") && message.includes("est_total=$0.25"),
      ),
    );
  } finally {
    service.close();
...
test("generateKeySummaries stores cached structured key summaries", async () => {
  ...
  try {
    ...
    const row = service.db.prepare("select provider, key_text from thread_key_summaries").get() as {
      provider: string;
      key_text: string;
    };
  ...
  try {
    ...
    service.db
      ...
      .run(
        ...
        "issue",
        "open",
        "gateway-client operator.read failure",
        "The gateway-client cannot call operator.read.",
        "alice",
        "User",
        "https://github.com/openclaw/openclaw/issues/42",
        "[]",
...
test("purgeComments removes hydrated comments and refreshes canonical documents", () => {
  ...
  try {
    ...
    const before = service.db
      .prepare("select dedupe_text from documents where thread_id = ?")
      .get(10) as { dedupe_text: string };
    assert.match(before.dedupe_text, /discussion:/);

    const result = service.purgeComments({ owner: "openclaw", repo: "openclaw" });

    const count = service.db.prepare("select count(*) as count from comments").get() as {
      count: number;
    };
  ...
  try {
    ...
    service.db
      ...
      .run(
        ...
        "v1",
        "summary-hash",
        "Transfer hangs near completion.",
        now,
        now,
      );
    ...
    assert.deepEqual(
      service.db
        ...
        .map((row: unknown) => {
          ...
          return {
            basis: typed.basis,
            vectorKind: Buffer.isBuffer(typed.vector_json) ? "blob" : typeof typed.vector_json,
          };
        }),
...
test("listNeighbors uses the vectorlite sidecar for current active vectors", async () => {
    ...
    ai: {
      ...
      embedTexts: async ({ texts }) =>
        texts.map((_text, index) =>
          index === 0 ? makeEmbedding(1, 0) : makeEmbedding(0.99, 0.01),
        ),
    },
  });
  ...
  try {
    ...
    insert.run(
      ...
      "issue",
      "open",
      "Downloader retry issue",
      "The transfer retries forever.",
      "bob",
      "User",
      "https://github.com/openclaw/openclaw/issues/43",
      "[]",
    ...
    assert.deepEqual(
      result.neighbors.map((neighbor) => neighbor.number),
      [43],
    );
  } finally {
    service.close();
...
test("embedRepository prunes closed vectors before reusing current active vectors", async () => {
  ...
  try {
    ...
    insertThread.run(
      ...
      "open",
      "Open fallback",
      "Somewhat similar body.",
      "dave",
      "User",
      "https://github.com/openclaw/openclaw/issues/45",
      "[]",
      "[]",
      "{}",
      "hash-45",
    ...
    const vectorCount = service.db
      .prepare("select count(*) as count from thread_vectors")
      .get() as { count: number };
...
test("embedRepository truncates oversized inputs before submission", async () => {
  ...
  try {
    ...
    const truncatedBodies = embedCalls
      .flat()
...
test("clusterRepository merges source kinds into one edge without directional duplicates", async () => {
  ...
  try {
    ...
    assert.deepEqual([edges[0]?.left_thread_id, edges[0]?.right_thread_id], [10, 11]);
    assert.deepEqual(JSON.parse(edges[0]?.explanation_json ?? "{}").sources, [
      "body",
      "dedupe_summary",
      "title",
    ]);
...
test("clusterRepository drops weak issue/pr semantic edges", async () => {
  ...
  try {
    ...
    insertThread.run(
      ...
      "Stale cache entries are not removed.",
      "carol",
      "User",
      "https://github.com/openclaw/openclaw/issues/44",
      "[]",
...
test("clusterRepository prunes older cluster runs for the repo after a successful rebuild", async () => {
  ...
  try {
    ...
    service.db
      .prepare(
        `insert into similarity_edges (repo_id, cluster_run_id, left_thread_id, right_thread_id, method, score, explanation_json, created_at) values (?, ?, ?, ?, ?, ?, ?, ?)`,
      )
    ...
    const runRows = service.db
      .prepare("select id from cluster_runs where repo_id = ? order by id asc")
      .all(1) as Array<{ id: number }>;
    const oldEdgeCount = service.db
      .prepare("select count(*) as count from similarity_edges where cluster_run_id = 1")
      .get() as { count: number };
...
test("clusterRepository purges legacy embeddings and inline vector payloads after a current-vector rebuild", async () => {
  ...
  try {
    ...
    await service.embedRepository({ owner: "openclaw", repo: "openclaw" });
    const beforeCluster = service.db
    ...
    const legacyCount = service.db
      .prepare("select count(*) as count from document_embeddings")
      .get() as { count: number };
    const inlineVectors = service.db
      .prepare(
        "select typeof(vector_json) as vector_kind from thread_vectors order by thread_id asc",
      )
      .all() as Array<{ vector_kind: string }>;

    assert.equal(legacyCount.count, 0);
    assert.deepEqual(
      inlineVectors.map((row) => row.vector_kind),
      ["blob", "blob"],
    );
  } finally {
    service.close();
...
test("clusterRepository rebuilds a corrupted active vector store and retries", async () => {
  ...
  const vectorStore: VectorStore = {
    ...
    queryNearest: ({ excludeThreadId }) => {
      if (firstQuery) {
        firstQuery = false;
        throw new Error(
          "Failed to load index from file: Index seems to be corrupted or unsupported",
        );
      }
      return [...vectors.keys()]
  ...
  try {
    ...
    const durableClusters = service.db
      .prepare("select count(*) as count from cluster_groups")
      .get() as { count: number };
    const durableMemberships = service.db
      .prepare("select count(*) as count from cluster_memberships")
      .get() as { count: number };
    const durableEdges = service.db
      .prepare("select count(*) as count from similarity_edge_evidence")
      .get() as { count: number };
    assert.equal(durableClusters.count, 1);
    assert.equal(durableMemberships.count, 2);
    assert.equal(durableEdges.count, 1);

    const cluster = service.db.prepare("select id from cluster_groups limit 1").get() as {
      id: number;
    };
    service.db
      .prepare(
    ...
    const blocked = service.db
      .prepare(
        "select state, removed_by from cluster_memberships where cluster_id = ? and thread_id = ?",
      )
  ...
  try {
    ...
    const durable = service as unknown as {
      persistDurableClusterState(
        ...
        aggregatedEdges: Map<
          string,
          { leftThreadId: number; rightThreadId: number; score: number; sourceKinds: Set<never> }
        >,
    ...
    durable.persistDurableClusterState(1, 1, noEdges, [
      { representativeThreadId: 10, members: [10, 11] },
    ]);
    const first = service.db
      .prepare("select id, stable_slug from cluster_groups limit 1")
      .get() as { id: number; stable_slug: string };

    durable.persistDurableClusterState(1, 2, noEdges, [
      { representativeThreadId: 11, members: [10, 11, 12] },
    ]);
    const groups = service.db
      .prepare(
        "select id, stable_slug, representative_thread_id from cluster_groups order by id asc",
      )
      .all() as Array<{
      id: number;
      stable_slug: string;
    ...
    const members = service.db
      .prepare(
        "select thread_id from cluster_memberships where cluster_id = ? order by thread_id asc",
      )
      .all(first.id) as Array<{ thread_id: number }>;

    assert.deepEqual(groups, [
      { id: first.id, stable_slug: first.stable_slug, representative_thread_id: 11 },
    ]);
    assert.deepEqual(
...
test("clusterRepository falls back to deterministic fingerprints when vectors are missing", async () => {
  ...
  try {
    ...
    insertThread.run(
      ...
      "issue",
      "open",
      "Download retry hangs forever",
      "The transfer retry loop never exits after timeout.",
      "alice",
      "User",
      "https://github.com/openclaw/openclaw/issues/42",
      "[]",
    ...
    insertThread.run(
      ...
      "issue",
      "open",
      "Download retry loop never exits",
      "Retry hangs forever after timeout.",
      "bob",
      "User",
      "https://github.com/openclaw/openclaw/issues/43",
      "[]",
    ...
    assert.equal(result.edges, 1);
    assert.equal(result.clusters, 1);
    const revisionCount = service.db
      .prepare("select count(*) as count from thread_revisions")
      .get() as { count: number };
    const fingerprintCount = service.db
      .prepare("select count(*) as count from thread_fingerprints")
      .get() as { count: number };
...
test("clusterRepository preserves a forced canonical representative on rebuild", async () => {
  ...
  try {
    ...
    const roles = service.db
      .prepare(
        "select thread_id, role, added_by from cluster_memberships where cluster_id = ? order by thread_id asc",
      )
...
test("clusterRepository preserves a forced include on rebuild", async () => {
  ...
  try {
    ...
    const membership = service.db
      .prepare(
        "select role, state, added_by from cluster_memberships where cluster_id = ? and thread_id = ?",
      )
...
test("mergeDurableClusters preserves source slug and force-includes active source members", () => {
  ...
  try {
    ...
    const source = service.db.prepare("select status from cluster_groups where id = ?").get(7) as {
      status: string;
    };
    const alias = service.db
      .prepare("select reason from cluster_aliases where cluster_id = ? and alias_slug = ?")
      .get(8, "source-slug") as {
      reason: string;
    };
    const override = service.db
      .prepare(
        "select action, reason from cluster_overrides where cluster_id = ? and thread_id = ?",
      )
      .get(8, 11) as {
      action: string;
      reason: string;
    ...
    const membership = service.db
      .prepare(
        "select state, added_by from cluster_memberships where cluster_id = ? and thread_id = ?",
      )
  ...
  try {
    ...
    assert.ok(
      secondMessages.some((message) =>
        message.includes("[fingerprint] latest revisions computed=0 skipped=2"),
      ),
    );

    service.db
      .prepare(
        "update threads set body = ?, content_hash = ?, updated_at_gh = ?, updated_at = ? where id = ?",
      )
      .run(
        "The transfer retry loop never exits after a network timeout.",
        "hash-42b",
    ...
    const fingerprintCount = service.db
      ...
      .get() as { count: number };
    assert.ok(
      thirdMessages.some((message) =>
        message.includes("[fingerprint] latest revisions computed=1 skipped=1"),
      ),
    );
...
test("clusterRepository can refresh one durable neighborhood without replacing the full snapshot", async () => {
  ...
  try {
    ...
    insertThread.run(
      ...
      "issue",
      "open",
      "Improve documentation typography",
      "Docs heading sizes look inconsistent.",
      "carol",
      "User",
      "https://github.com/openclaw/openclaw/issues/44",
      "[]",
    ...
    const full = await service.clusterRepository({
      owner: "openclaw",
      repo: "openclaw",
      k: 1,
      minScore: 0.1,
    ...
    const incrementalRun = service.db
      .prepare(
        "select run_kind from pipeline_runs where run_kind = 'cluster_incremental' order by id desc limit 1",
      )
      .get() as { run_kind: string } | undefined;

    assert.ok(
      messages.some((message) =>
        message.includes("[fingerprint] latest revisions computed=1 skipped=0"),
      ),
    );
    assert.ok(
      messages.some((message) => message.includes("without replacing the full cluster snapshot")),
    );
...
test("clusterRepository keeps deterministic hunk edges when active vectors are current", async () => {
  const service = new GHCrawlService({
    ...
    ai: {
      ...
      embedTexts: async ({ texts }) =>
        texts.map((_text, index) => (index === 0 ? makeEmbedding(1, 0) : makeEmbedding(0, 1))),
    },
  });
  ...
  try {
    ...
    const evidence = service.db
      .prepare("select breakdown_json from similarity_edge_evidence")
      .get() as { breakdown_json: string };
    assert.equal(result.edges, 1);
...
test("clusterRepository can reuse stale active vectors for offline reclustering", async () => {
  ...
  try {
    ...
    await service.embedRepository({ owner: "openclaw", repo: "openclaw" });
    service.db
      .prepare("update repo_pipeline_state set summary_model = 'previous-model' where repo_id = 1")
      .run();
    ...
    const state = service.db
      .prepare("select clusters_current_at from repo_pipeline_state where repo_id = 1")
      .get() as {
...
test("tui snapshot returns mixed issue and pull request counts with default visible cluster filter", () => {
  ...
  try {
    ...
    insertThread.run(
      ...
      "erin",
      "User",
      "https://github.com/openclaw/openclaw/pull/46",
      "[]",
      "[]",
      "{}",
      "hash-46",
    ...
    service.db
      .prepare(
        `insert into sync_runs (id, repo_id, scope, status, started_at, finished_at) values (?, ?, ?, ?, ?, ?)`,
      )
      .run(1, 1, "openclaw/openclaw", "completed", now, "2026-03-09T12:00:00Z");
    service.db
      .prepare(
        `insert into embedding_runs (id, repo_id, scope, status, started_at, finished_at) values (?, ?, ?, ?, ?, ?)`,
      )
    ...
    assert.match(
      allSnapshot.clusters[0]?.displayTitle ?? "",
      /^[a-z]+-[a-z]+-[a-z]+  Recent issue cluster$/,
    );

    const sizeSorted = service.getTuiSnapshot({
      owner: "openclaw",
      repo: "openclaw",
      minSize: 0,
      sort: "size",
    ...
    const filtered = service.getTuiSnapshot({
      owner: "openclaw",
      repo: "openclaw",
      minSize: 0,
      search: "old pr cluster",
    });
    assert.deepEqual(
  ...
  try {
    ...
    service.db
      ...
      .run(
        ...
        "gpt-5-mini",
        "hash-problem",
        "Downloads hang before completion.",
        now,
        now,
      );
    service.db
      ...
      .run(
        ...
        "gpt-5-mini",
        "hash-dedupe",
        "Transfer stalls near completion.",
        now,
        now,
      );
    service.db
    ...
    const threadDetail = service.getTuiThreadDetail({
      owner: "openclaw",
      repo: "openclaw",
      threadNumber: 42,
    });
...
test("agent cluster summary and detail dumps expose repo stats, snippets, and summaries", () => {
  ...
  try {
    ...
    assert.match(
      summaries.clusters[0]?.displayTitle ?? "",
      /^[a-z]+-[a-z]+-[a-z]+  Downloader hangs$/,
    );
...
test("local thread closure updates default thread filters and auto-closes fully closed clusters", () => {
  ...
  try {
    ...
    const firstClose = service.closeThreadLocally({
      owner: "openclaw",
      repo: "openclaw",
      threadNumber: 42,
    });
    ...
    assert.deepEqual(
      service
        .listThreads({ owner: "openclaw", repo: "openclaw" })
        .threads.map((thread) => thread.number),
      [43],
    );
    assert.deepEqual(
      service
        .listThreads({ owner: "openclaw", repo: "openclaw", includeClosed: true })
        .threads.map((thread) => thread.number),
      [43, 42],
    );

    const secondClose = service.closeThreadLocally({
      owner: "openclaw",
      repo: "openclaw",
      threadNumber: 43,
    });
    ...
    const hiddenSummaries = service.listClusterSummaries({
      owner: "openclaw",
      repo: "openclaw",
      minSize: 0,
      includeClosed: false,
  ...
  try {
    ...
    service.db
      ...
      .run(
        7,
        1,
        durableIdentity.hash,
        durableIdentity.slug,
        "active",
        "duplicate_candidate",
        10,
        "Durable cluster",
    ...
    const response = service.closeClusterLocally({
      owner: "openclaw",
      repo: "openclaw",
      clusterId: 100,
    });
    ...
    const durable = service.db
      .prepare("select status, closed_at from cluster_groups where id = ?")
      .get(7) as {
      status: string;
      closed_at: string | null;
    };
    assert.deepEqual(durable, { status: "active", closed_at: null });
    const closure = service.db
      .prepare("select reason, actor_kind from cluster_closures where cluster_id = ?")
      .get(7) as {
      reason: string;
      actor_kind: string;
    };
    ...
    assert.equal(
      service.listClusterSummaries({ owner: "openclaw", repo: "openclaw", minSize: 0 }).clusters
        .length,
      1,
    );
    const detail = service.getClusterDetailDump({
    ...
    const prunedSnapshot = service.getTuiSnapshot({
      owner: "openclaw",
      repo: "openclaw",
      minSize: 0,
    });
  ...
  try {
    ...
    service.db
      ...
      .run(
        9,
        1,
        "stable-key-duplicate-archived",
        "archive-blue-duplicate",
        "active",
        "duplicate_candidate",
        11,
        "Duplicate archived durable cluster",
        now,
        now,
        null,
      );
    ...
    const hidden = service.getTuiSnapshot({
      owner: "openclaw",
      repo: "openclaw",
      minSize: 0,
      includeClosedClusters: false,
    });
...
test("excludeThreadFromCluster records a durable manual exclusion", () => {
  ...
  try {
    ...
    const event = service.db
      .prepare("select event_type, actor_kind from cluster_events where cluster_id = ?")
      .get(7) as {
      event_type: string;
      actor_kind: string;
    };
  ...
  try {
    ...
    service.db
      ...
      .run(
        ...
        10,
        11,
        "persistent-cluster-v1",
        "config",
        0.91,
        "strong",
        "active",
        '{"sources":["deterministic_fingerprint"],"score":0.91}',
        null,
        null,
        now,
        now,
    ...
    service.db
      .prepare(
        "insert into cluster_overrides (repo_id, cluster_id, thread_id, action, reason, created_at, expires_at) values (?, ?, ?, ?, ?, ?, ?)",
      )
      .run(1, 7, 10, "force_canonical", "best root issue", now, null);
    service.db
      .prepare(
        "insert into cluster_aliases (cluster_id, alias_slug, reason, created_at) values (?, ?, ?, ?)",
      )
      .run(7, "old-slug", "merged_from:3", now);
    service.db
      .prepare(
        "insert into cluster_events (cluster_id, run_id, event_type, actor_kind, payload_json, created_at) values (?, ?, ?, ?, ?, ?)",
      )
      .run(7, null, "keep_canonical", "algo", '{"threadId":10}', now);

    const response = service.explainDurableCluster({
      owner: "openclaw",
      repo: "openclaw",
      clusterId: 7,
    });
...
test("syncRepository keeps source author fields without building actor profiles", async () => {
  ...
  try {
    ...
    const thread = service.db
      .prepare("select author_login, author_type from threads where number = 42")
      .get() as {
      author_login: string;
      author_type: string;
    };
    const comment = service.db
      .prepare("select author_login, author_type from comments where github_id = ?")
      .get("900") as {
      author_login: string;
      author_type: string;
    };
...
test("syncRepository reconciles stale open threads and marks confirmed closures without re-fetching comments", async () => {
  ...
  try {
    ...
    const before = service.db
      .prepare(
        "select state, first_pulled_at, last_pulled_at from threads where number = 42 and kind = 'issue'",
      )
    ...
    const after = service.db
      .prepare(
        "select state, closed_at_gh, first_pulled_at, last_pulled_at from threads where number = 42 and kind = 'issue'",
      )
      .get() as {
      state: string;
...
test("syncRepository treats missing stale pull requests as closed and continues", async () => {
  ...
  const service = makeTestService({
    ...
    getPull: async () => {
      getPullCalls += 1;
      throw Object.assign(
        new Error("GitHub request failed for GET /repos/openclaw/openclaw/pulls/43: Not Found"),
        {
          status: 404,
        },
      );
  ...
  try {
    ...
    const after = service.db
      .prepare(
        "select state, closed_at_gh, last_pulled_at from threads where number = 43 and kind = 'pull_request'",
      )
      .get() as {
      state: string;
      closed_at_gh: string | null;
...
test("syncRepository fullReconcile backfills stale closed items from closed pages before direct checks", async () => {
  ...
  try {
    ...
    const statsRow = service.db
      .prepare(
        "select stats_json from sync_runs where status = 'completed' order by id desc limit 1",
      )
...
test("syncRepository derives the default overlapping since window from the last completed full scan", async () => {
  ...
  try {
    ...
    assert.ok(
      Date.parse(syncState.last_overlapping_open_scan_completed_at) >=
        Date.parse("2026-03-09T14:13:01.000Z"),
    );
    assert.equal(syncState.last_non_overlapping_scan_completed_at, null);
    assert.equal(
      syncState.last_open_close_reconciled_at,
      syncState.last_overlapping_open_scan_completed_at,
    );
  } finally {
    service.close();
  }
...
test("repository-scoped reads and neighbors do not leak across repos in the same database", () => {
  ...
  try {
    ...
    insertThread.run(
      ...
      "alice",
      "User",
      "https://github.com/owner-one/repo-one/issues/42",
      "[]",
      "[]",
      "{}",
      "hash-10",
    ...
    insertThread.run(
      ...
      "bob",
      "User",
      "https://github.com/owner-one/repo-one/pull/43",
      "[]",
      "[]",
      "{}",
      "hash-11",
    ...
    insertThread.run(
      ...
      "carol",
      "User",
      "https://github.com/owner-two/repo-two/issues/42",
      "[]",
      "[]",
      "{}",
      "hash-20",
    ...
    service.db
      .prepare(
        `insert into clusters (id, repo_id, cluster_run_id, representative_thread_id, member_count, created_at) values (?, ?, ?, ?, ?, ?)`,
      )
      .run(100, 1, 1, 10, 2, now);
    service.db
      .prepare(
    ...
    const repoOneSnapshot = service.getTuiSnapshot({
      owner: "owner-one",
      repo: "repo-one",
      minSize: 0,
    });
