...
export type GitcrawlClient = {
  ...
  closeCluster: (request: {
    owner: string;
    repo: string;
    clusterId: number;
  }) => Promise<CloseResponse>;
  excludeClusterMember: (request: {
    owner: string;
    repo: string;
    clusterId: number;
    threadNumber: number;
    reason?: string;
  }) => Promise<ClusterOverrideResponse>;
  includeClusterMember: (request: {
    owner: string;
    repo: string;
    clusterId: number;
    threadNumber: number;
  ...
  mergeClusters: (request: {
    owner: string;
    repo: string;
    sourceClusterId: number;
    targetClusterId: number;
    reason?: string;
  }) => Promise<ClusterMergeResponse>;
  splitCluster: (request: {
    owner: string;
    repo: string;
    sourceClusterId: number;
    threadNumbers: number[];
    reason?: string;
  }) => Promise<ClusterSplitResponse>;
};
  ...
  return {
    ...
    async listThreads(params) {
      ...
      if (params.numbers && params.numbers.length > 0)
        search.set("numbers", params.numbers.join(","));
    ...
    async listClusters(params) {
      ...
      if (params.includeClosed !== undefined)
        search.set("includeClosed", String(params.includeClosed));
