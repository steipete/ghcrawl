...
test("missing required flags exit with code 2 and command-specific hints", async () => {
  const cases = [
    {
      argv: ["close-thread", "openclaw/openclaw"],
      message: /Missing --number/,
      hint: /Run 'ghcrawl close-thread --help' for usage\./,
    },
    {
      argv: ["cluster-detail", "openclaw/openclaw"],
      message: /Missing --id/,
      hint: /Run 'ghcrawl cluster-detail --help' for usage\./,
    },
  ];
  ...
  try {
    {
      ...
      const code = await runCli(
        ["search", "openclaw/openclaw", "--query", "download stalls", "--mode", "bogus"],
        {
...
test("exclude-cluster-member command forwards durable override inputs", async () => {
  ...
  GHCrawlService.prototype.excludeThreadFromCluster = function excludeThreadFromClusterStub(
    params: unknown,
  ) {
    received = params;
    return {
...
test("set-cluster-canonical command forwards durable override inputs", async () => {
  ...
  GHCrawlService.prototype.setClusterCanonicalThread = function setClusterCanonicalThreadStub(
    params: unknown,
  ) {
    received = params;
    return {
...
test("include-cluster-member command forwards durable override inputs", async () => {
  ...
  GHCrawlService.prototype.includeThreadInCluster = function includeThreadInClusterStub(
    params: unknown,
  ) {
    received = params;
    return {
...
test("merge-clusters command forwards durable merge inputs", async () => {
  ...
  GHCrawlService.prototype.mergeDurableClusters = function mergeDurableClustersStub(
    params: unknown,
  ) {
    received = params;
    return {
...
test("cluster command forwards neighborhood refresh inputs", async () => {
  ...
  GHCrawlService.prototype.clusterRepository = async function clusterRepositoryStub(
    params: unknown,
  ) {
    received = params;
...
test("clusters command shows closed clusters by default and forwards hide-closed", async () => {
  ...
  GHCrawlService.prototype.listClusterSummaries = function listClusterSummariesStub(
    params: unknown,
  ) {
    received.push(params);
...
test("durable-clusters command forwards stable cluster list options", async () => {
  ...
  GHCrawlService.prototype.listDurableClusters = function listDurableClustersStub(params: unknown) {
    ...
    return {
      ...
      clusters: [{ stableSlug: "trace-alpha-river" }],
    } as never;
  };

  try {
    await run(
      ["durable-clusters", "openclaw/openclaw", "--include-inactive", "--member-limit", "5"],
      stdout.stream,
      {
        env: context.env,
        cwd: context.cwd,
...
test("cluster-explain command forwards evidence options", async () => {
  ...
  GHCrawlService.prototype.explainDurableCluster = function explainDurableClusterStub(
    params: unknown,
  ) {
    received = params;
    return {
...
test("sync-status command forwards portable path", async () => {
  ...
  try {
    await run(
      ["sync-status", "openclaw/openclaw", "--portable", "/tmp/openclaw.sync.db"],
      stdout.stream,
      {
        env: context.env,
        cwd: context.cwd,
...
test("refresh command forwards include-code hydration flag", async () => {
  ...
  GHCrawlService.prototype.refreshRepository = async function refreshRepositoryStub(
    params: unknown,
  ) {
    received = params;
    return {
...
test("key-summaries command forwards enrichment options", async () => {
  ...
  GHCrawlService.prototype.generateKeySummaries = async function generateKeySummariesStub(
    params: unknown,
  ) {
    received = params;
    return {
  ...
  try {
    await run(
      ["key-summaries", "openclaw/openclaw", "--number", "42", "--limit", "1"],
      stdout.stream,
      {
        env: context.env,
        cwd: context.cwd,
...
test("parseRepoFlags accepts exclusion reason", () => {
  const parsed = parseRepoFlags("exclude-cluster-member", [
    "openclaw/openclaw",
    "--id",
    "7",
    "--number",
...
test("parseRepoFlags accepts heap diagnostics options", () => {
  const parsed = parseRepoFlags("cluster", [
    "openclaw/openclaw",
    "--heap-snapshot-dir",
    "./tmp/heaps",
    "--heap-log-interval-ms",
    "5000",
  ]);
  assert.equal(parsed.owner, "openclaw");
  assert.equal(parsed.repo, "openclaw");
...
test("formatLogLine prefixes ISO timestamps with millisecond resolution", () => {
  assert.equal(
    formatLogLine("[sync] hello", new Date("2026-03-09T12:34:56.789Z")),
    "[2026-03-09T12:34:56.789Z] [sync] hello",
  );
});
