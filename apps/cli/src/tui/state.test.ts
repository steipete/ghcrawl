...
test("applyClusterFilters sorts by recent and size and respects min size/search", () => {
  ...
  assert.deepEqual(
    applyClusterFilters(clusters, { sortMode: "recent", minSize: 10, search: "" }).map(
      (cluster) => cluster.clusterId,
    ),
    [2, 1],
  );
  assert.deepEqual(
    applyClusterFilters(clusters, { sortMode: "size", minSize: 10, search: "" }).map(
      (cluster) => cluster.clusterId,
    ),
    [1, 2],
  );
  ...
  assert.deepEqual(
    applyClusterFilters(clusters, { sortMode: "recent", minSize: 0, search: "newest" }).map(
      (cluster) => cluster.clusterId,
    ),
    [2],
  );
