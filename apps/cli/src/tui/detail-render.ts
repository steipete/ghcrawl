...
export function renderDetailPane(
  ...
  if (!clusterDetail) {
    ...
    return [
      ...
      "",
      clusterCount > 0
        ? `${clusterCount} clusters loaded. Click a cluster or press Enter to inspect members.`
        : "No clusters visible in this view.",
      "",
      `{bold}Controls{/bold}`,
  ...
  const labels =
    thread.labels.length > 0
      ? thread.labels.map((label) => `{cyan-fg}${escapeBlessedText(label)}{/cyan-fg}`).join(" ")
      : "none";
  ...
  const neighbors =
    threadDetail.neighbors.length > 0
      ? threadDetail.neighbors
          .map(
            (neighbor) =>
              `#${neighbor.number} ${neighbor.kind} ${(neighbor.score * 100).toFixed(1)}%  ${escapeBlessedText(neighbor.title)}`,
          )
          .join("\n")
      : focusPane === "detail"
        ? "No neighbors available."
        : "Neighbors load when the detail pane is focused.";
  const body = limitRenderedLines(
    renderMarkdownForTerminal(thread.body ?? "(no body)"),
    detailMode === "compact" ? 18 : 240,
  );
...
export function getThreadReferenceLinks(threadDetail: TuiThreadDetail | null): string[] {
  ...
  return uniqueStrings([
    ...
    ...Object.values(threadDetail.summaries).flatMap((summary) =>
...
export function formatThreadDetailForClipboard(
  threadDetail: TuiThreadDetail,
  clusterDetail: TuiClusterDetail | null,
): string {
  const thread = threadDetail.thread;
  ...
  const sections = [
    ...
    clusterDetail && clusterTitle
      ? `Cluster ${clusterDetail.clusterId}: ${clusterTitle.name} | ${clusterTitle.title}`
      : "",
...
export function formatClusterForClipboard(cluster: TuiClusterDetail): string {
  ...
  return [
    ...
    cluster.representativeNumber !== null
      ? `Representative: #${cluster.representativeNumber} ${cluster.representativeKind ?? ""}`.trimEnd()
      : "",
  ]
    .filter(Boolean)
    .join("\n");
...
const SUMMARY_SECTION_ORDER: SummaryKey[] = [
  "problem_summary",
  "solution_summary",
...
function renderInlineMarkdown(value: string): string {
  ...
  return segments
    .map((segment) =>
      segment.kind === "link"
        ? formatTerminalLink(segment.url, segment.label)
        : renderInlineText(segment.value),
    )
    .join("");
}
