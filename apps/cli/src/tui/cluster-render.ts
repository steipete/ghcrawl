...
export function formatClusterListLabel(cluster: TuiClusterSummary): string {
  ...
  const mixLabel = `${cluster.issueCount}I/${cluster.pullRequestCount}P`.padStart(
    CLUSTER_MIX_WIDTH,
  );
...
export function resolveClusterHeaderSortFromClick(
  ...
  const visibleUpdatedStart = Math.min(
    CLUSTER_UPDATED_START,
    Math.max(CLUSTER_NAME_START, visibleWidth - CLUSTER_UPDATED_WIDTH - CLUSTER_COLUMN_GAP),
  );
...
export function formatClusterDateColumn(
  value: string | null,
