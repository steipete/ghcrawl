...
export function getRepositoryChoices(
  service: Pick<GHCrawlService, "listRepositories">,
  now: Date = new Date(),
): RepositoryChoice[] {
  const repositories = service
    .listRepositories()
    .repositories.slice()
    .sort(
      (left, right) =>
...
export async function promptRepositoryInput(
  screen: blessed.Widgets.Screen,
): Promise<RepositoryTarget | null> {
