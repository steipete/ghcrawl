...
export function createScreen(
  options: Parameters<typeof blessed.screen>[0],
...
export function updatePaneStyles(widgets: Widgets, focus: TuiFocusPane): void {
  ...
  widgets.clusters.style.selected =
    focus === "clusters"
      ? { bg: "#f7f7ff", fg: "black", bold: true }
      : { bg: "#23445c", fg: "white", bold: true };
  widgets.members.style.selected =
    focus === "members"
      ? { bg: "#f7f7ff", fg: "black", bold: true }
      : { bg: "#33521e", fg: "white", bold: true };
}

export function applyRect(
  element: blessed.Widgets.BoxElement | blessed.Widgets.ListElement,
...
export function getListItemIndexFromMouse(
