...
export function parseRepoFlags(command: CommandName, args: string[]): ParsedRepoFlags {
  ...
  throw new CliUsageError(
    "Use --repo owner/repo or provide owner/repo as the first positional argument",
    command,
  );
}
...
export function resolveSinceValue(value: string, now: Date = new Date()): string {
  ...
  if (!match) {
    throw new CliUsageError(
      `Invalid --since value: ${value}. Use an ISO timestamp or duration like 15m, 2h, 7d, or 1mo.`,
    );
  }
...
export function parseEnum<T extends string>(
  ...
  throw new CliUsageError(
    `Invalid --${flagName}: ${value}. Use one of ${allowed.join(", ")}.`,
    command,
  );
}
