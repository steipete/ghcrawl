...
const TEST_THREAD_IDS = [
  // Issues
  15126, 8920, 19616, 16324, 10106, 14855, 18179, 2538, 9401, 9156, 18848, 14856, 14863, 18847,
  5022, 14862, 14859, 14142, 14861, 21902,
  // PRs
  22366, 17692, 20932, 13791, 4208, 9553, 8969, 17568, 4129, 21735, 2463, 5418, 5796, 766, 17924,
  5712, 21769, 8098, 539, 5565,
];
...
async function judgeResult(client, judgeModel, title, body, dedupeSummary) {
  ...
  try {
    ...
    return {
      score: null,
      has_boilerplate: null,
      signal_density: null,
      would_cluster_correctly: null,
...
try {
  service.requireRepository(args.owner, args.repo);
  ...
  for (const [index, threadId] of TEST_THREAD_IDS.entries()) {
    const thread = service.db
      .prepare("SELECT id, number, kind, title, body, labels_json FROM threads WHERE id = ?")
      .get(threadId);

    if (!thread) {
      process.stderr.write(
        `[${index + 1}/${TEST_THREAD_IDS.length}] thread ${threadId} not found, skipping\n`,
      );
      continue;
    }
    ...
    process.stderr.write(
      `[${index + 1}/${TEST_THREAD_IDS.length}] #${thread.number} (${thread.kind}) summarizing...\n`,
    );
    ...
    try {
      ...
      process.stderr.write(
        `[${index + 1}/${TEST_THREAD_IDS.length}] #${thread.number} judging...\n`,
      );
      const judgeResult_ = await judgeResult(
        client,
        judgeModel,
        title,
        thread.body,
        summaryResult.summary.dedupe_summary,
      );
      ...
      process.stderr.write(
        `[${index + 1}/${TEST_THREAD_IDS.length}] #${thread.number} ERROR: ${error.message}\n`,
      );
      results.push({
        thread_id: threadId,
        number: thread.number,
  ...
  const avgScore =
    scored.length > 0 ? scored.reduce((s, r) => s + r.judge.score, 0) / scored.length : 0;
  const avgSignalDensity =
    scored.length > 0 ? scored.reduce((s, r) => s + r.judge.signal_density, 0) / scored.length : 0;
  ...
  const experiment = {
    ...
    aggregate: {
      ...
      would_cluster_correctly_pct: Math.round(
        (wouldClusterCount / Math.max(scored.length, 1)) * 100,
      ),
