...
const EXPERIMENTS = [
  ...
  {
    id: "source-title-dedupe",
    args: ["--source-kinds", "title,dedupe_summary", "--aggregation", "max"],
  },
  {
    id: "source-body-dedupe",
    args: ["--source-kinds", "body,dedupe_summary", "--aggregation", "max"],
  },
  ...
  {
    id: "agg-weighted-heavy-summary",
    args: [
      "--aggregation",
      "weighted",
      "--weights",
      '{"dedupe_summary":0.7,"title":0.2,"body":0.1}',
    ],
  },
  ...
  {
    id: "param-large-clusters",
    args: ["--source-kinds", "dedupe_summary", "--max-cluster-size", "400"],
  },

  // Best combos (will add based on early results)
  {
    id: "combo-dedupe-weighted-low",
    args: [
      "--source-kinds",
      "title,dedupe_summary",
      "--aggregation",
      "weighted",
      "--threshold",
      "0.78",
...
const summaryTable = [];
const commonArgs = ["openclaw/openclaw", "--cluster-mode", "bounded", "--max-cluster-size", "200"];

for (const experiment of EXPERIMENTS) {
  if (existing.has(experiment.id)) {
    try {
      const result = JSON.parse(
        fs.readFileSync(path.join(resultsDir, `${experiment.id}.json`), "utf8"),
      );
