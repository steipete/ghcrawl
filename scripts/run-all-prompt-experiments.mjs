...
for (const file of promptFiles) {
  ...
  if (existing.has(experimentId)) {
    ...
    const result = JSON.parse(
      fs.readFileSync(path.join(resultsDir, `${experimentId}.json`), "utf8"),
    );
    const scored = result.results.filter((r) => r.judge?.score != null);
    if (scored.length >= 30) {
      process.stderr.write(
        `[SKIP] ${experimentId} — already has ${scored.length} scored results\n`,
      );
      summaryTable.push({ experiment_id: experimentId, ...result.aggregate, status: "cached" });
      continue;
    }
    process.stderr.write(
      `[RERUN] ${experimentId} — only ${scored.length} scored results, rerunning\n`,
    );
  }
